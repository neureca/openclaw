...
## Local Operator Addendum (openclaw VM)

This section defines how the assistant should operate in this specific deployment (`/home/openclaw/openclaw-2026.3.13-1`).

### Scope and defaults

- Primary repo for active changes: `/home/openclaw/openclaw-2026.3.13-1`.
- Runtime model: OpenAI Responses with incremental turns (`previous_response_id`) and persisted state (`store: true`).
- Gateway is expected to run as a user service (`openclaw-gateway.service`) on loopback port `18789`.
- Telegram is a production channel; treat delayed/empty replies as incidents, not cosmetic issues.

### Incident triage order (Telegram "typing but no response")

1. Verify gateway health (`status`, port listen, fresh logs).
2. Verify model/auth health (`openclaw models status`, active auth profile).
3. Verify channel connectivity (`openclaw channels status --probe`).
4. Check queue/timeout pressure (stuck runs, long tool loops, cron overlap).
5. Confirm DNS/connectivity from VM before changing app logic.

### Reliability guardrails

- Keep task timeout defaults conservative for this host (5 minutes baseline unless explicitly changed).
- Prefer incremental context transfer over full-history replay.
- Do not disable storage for Responses provider by default.
- Avoid introducing blocking retries with long backoff in Telegram send paths without bounded caps.

### Config and UI access notes

- Control UI origin must be explicitly allowed in `~/.openclaw/openclaw.json`:
  - `http://127.0.0.1:18789`
  - `http://localhost:18789`
- "gateway token missing" means operator must paste dashboard token into Control UI settings.
- "pairing required" means pending device/operator approval is needed before control actions are accepted.

### Safe change policy for this deployment

- Prefer minimal, reversible changes with targeted tests.
- When updating runtime-critical behavior (transport, timeout, delivery), run focused tests before restart.
- Do not batch unrelated refactors into incident-response fixes.
