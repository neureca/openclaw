...
## Project Overview (for this VM deployment)

This repository powers a self-hosted assistant running on a local VM:

- **Gateway**: local control plane (`127.0.0.1:18789`) for sessions, tools, web UI, and channel routing.
- **Channels**: Telegram (primary), plus optional Slack/Discord/WhatsApp/etc.
- **Model runtime**: OpenAI Responses with incremental turn continuation (`previous_response_id`) and persisted response state (`store: true`).
- **Automation**: cron and background tasks with bounded execution timeout (5 minutes default in this environment).

Operationally, the most important path is:

1. inbound Telegram message
2. routed into Gateway session
3. model/tool execution
4. outbound reply delivery back to Telegram

If the bot shows `typing` but does not reply, treat it as a runtime delivery incident and check gateway health, model auth, channel probe status, and timeout/queue pressure first.
